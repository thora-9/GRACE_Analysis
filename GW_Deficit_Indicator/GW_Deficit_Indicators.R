...
plot1 = 
  rasterFromXYZ(gws.OG.unique[,.(Lon, Lat, ISO_N3)])
...
GGDI = 
  ...
  as.data.table() %>%
  .[,`:=`(GWS.def.roll12 = frollapply(GWS.deficit, 12, mean),
          GWS.def.roll18 = frollapply(GWS.deficit, 18, mean),
          GWS.def.roll24 = frollapply(GWS.deficit, 24, mean)),] %>%
  as.data.table()
...
GGDI.binary1 = 
  GGDI %>%
  filter(year>=2019) %>%
  dplyr::select(cell_id, GWS.deficit) %>%
  group_by(cell_id) %>%
...
GGDI.binary2 = 
  GGDI %>%
  filter(year<=2017) %>%
  dplyr::select(cell_id, GWS.def.roll12, GWS.def.roll18, GWS.def.roll24) %>%
  group_by(cell_id) %>%
  ...
  summarise(Def.total12 = sum(GWS.binary12),
            Def.total18 = sum(GWS.binary18),
            Def.total24_150 = sum(GWS.binary24_150),
            Def.total24_125 = sum(GWS.binary24_125)) %>%
...
GGDI.out = 
  GGDI.binary1 %>%
  merge(GGDI.binary1b[,.(cell_id, Def.13_14)], by = 'cell_id', all.x = T) %>%
  merge(GGDI.binary2, by = 'cell_id', all.x = T) %>%
  merge(gws.OG.unique, by = 'cell_id', all.x = T) %>%
...
write.dta(GGDI.out.sub, paste0(pathOut, 'GGDI_output_nonDownscaled.dta'))
