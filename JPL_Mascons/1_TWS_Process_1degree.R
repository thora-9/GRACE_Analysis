...
tws_mascon =
  tidync(paste0(proj_dir,'GRACE_data/JPL_Mascons/GRCTellus.JPL.200204_202106.GLO.RL06M.MSCNv02CRI_1degree.nc')) %>%
  activate("D2,D3,D0") %>%
...
#Adjustment factors
clm_factors = 
  tidync(paste0(proj_dir,'GRACE_data/JPL_Mascons/CLM4.SCALE_FACTOR.JPL.MSCNv02CRI_1degree.nc')) %>%
  activate("D0,D1") %>%
  hyper_tibble() %>% as.data.table()
...
tws_mascon = 
  ...
  merge(clm_factors, by = c("lat", "lon")) %>%
...
fwrite(tws_lwe_base,
       paste0(proj_dir, "GRACE_Data/JPL_Mascons/GRACE_JPL_TWS_1degree_220725.csv"))
